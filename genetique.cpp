...
void algorithme_genetique(int max_generation, int taille_population, int nombre_villes, vector<vector<double>> matrice_des_distances, double frequence_mutation, int nombre_parents_survivants, int max_stagnation_generations) {
    ...
    double previousBestFitness = P.majorant().adaptation;  // Initialisation du meilleur fitness
    int stagnationCount = 0;  // Compteur de stagnation
    int i = 0;
    ...
    while(i < max_generation) {
        ...
        double currentBestFitness = P.majorant().adaptation;  // Récupère le fitness de la meilleure solution
        if (abs(currentBestFitness - previousBestFitness) <= 1e-6) {
            stagnationCount++;  // Augmente le compteur de stagnation
        } else {
            stagnationCount = 0;  // Réinitialise si une amélioration est détectée
        }

        // Si la stagnation dure trop longtemps, on arrête
        ...
        previousBestFitness = currentBestFitness;  // Met à jour le meilleur fitness

        i++;  // Passe à la génération suivante
    }
